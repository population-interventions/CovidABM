...
def MakeCostsMidFunction(contents, singleList, variables, singleAverages, singleMetricNoDiff):
	...
	for singleRange in singleList:
		...
		for var in variables:
			...
			if var not in singleAverages:
				if start == 0 or var in singleMetricNoDiff:
					contents.append('    set {} {}\n'.format(param, var))
				else:
...
def EditCostsOutput(modelDir, singleList, variables, singleAverages, singleMetricNoDiff):
	...
	while True:
		...
		if line.find('to CalculateMidCosts') > -1:
			...
			MakeCostsMidFunction(contents, singleList, variables, singleAverages, singleMetricNoDiff)
			contents.append('end\n')
			break
		contents.append(line + '\n')
...
def MakeExtraSingleVariables(modelDir, singleList, singleAverages, singleMetricNoDiff):
	variables = FindVariablesAndEditGlobals(modelDir, singleList)
	EditBehaviourSpace(modelDir, singleList, variables)
	EditCostsOutput(modelDir, singleList, variables, singleAverages, singleMetricNoDiff)
